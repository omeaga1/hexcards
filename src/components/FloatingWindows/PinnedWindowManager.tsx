...
export const PinnedWindowManager: React.FC = () => {
  ...
  if (frozenCards.length === 0 && !activeHoverTarget) return null;

  return (
    <div className="fixed inset-0 pointer-events-none z-50">
      {/* 1. Live Floating Hover Card Preview */}
      {activeHoverTarget && !frozenCards.some((c) => c.id.includes(activeHoverTarget.id)) && (
      ...
      {frozenCards.length >= 2 && (
        <div className="fixed bottom-4 right-4 pointer-events-auto">
          <button
            ...
            className="px-3 py-1 rounded bg-[#18231c]/90 hover:bg-rose-950/80 border border-[#2f4234] hover:border-rose-500 text-xs font-black uppercase text-[#cbdad0] hover:text-white shadow-xl transition-all font-['Barlow_Condensed'] flex items-center gap-1.5 backdrop-blur-sm cursor-pointer"
          >
